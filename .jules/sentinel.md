...
## 2026-06-29 - Missing Timeouts on External Network Calls (DoS)
**Vulnerability:** The application was establishing connections and sending requests to the external Ollama backend without specifying a timeout parameter in `ollama.Client(host=...)`.
**Learning:** Network requests without explicit timeouts can block the main thread indefinitely if the backend service hangs or drops packets without closing the connection. In a Streamlit environment, this leads to workers being tied up forever, culminating in a Denial of Service (DoS) for the application. Even for LLM APIs that might require longer processing times, a sensible upper bound (e.g., 30-120 seconds depending on the model size) must always be set.
**Prevention:** Always include explicit `timeout` arguments when creating clients or making HTTP requests to external services to ensure the application fails fast and recovers cleanly.
