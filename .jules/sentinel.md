...
## 2026-07-29 - [MEDIUM] Fix LLM streaming OOM DoS risk\n**Vulnerability:** Infinite LLM response length during streaming can lead to memory exhaustion (OOM DoS).\n**Learning:** When using infinite stream clients, unbounded accumulation of chunk outputs creates memory risks.\n**Prevention:** Always enforce a max length buffer limitation inside while/for loops during streaming data accumulation.
