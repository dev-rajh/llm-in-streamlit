...
## 2026-06-28 - Backend Resource Exhaustion via Chat Input Spam
...
## 2024-05-18 - Unbounded Stream Response OOM
**Vulnerability:** The application was vulnerable to an Out-Of-Memory (OOM) Denial of Service (DoS) attack because responses streamed from the Ollama LLM were accumulated in a string without any upper bounds check.
**Learning:** Even if the input constraints and context are small, the LLM itself could enter an infinite loop or stream excessively long responses if compromised, misconfigured, or given malicious instructions. String concatenation in a `while True`-like loop without bounds will eventually exhaust server memory and crash the application.
**Prevention:** To prevent OOM vulnerabilities when dealing with infinite or excessively large LLM streams, always enforce a strict `MAX_RESPONSE_LENGTH` inside the stream accumulation loop and truncate/break out of the loop gracefully if the limit is exceeded.
