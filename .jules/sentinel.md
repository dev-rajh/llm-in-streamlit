...
## 2025-02-14 - Prevent Application DoS by Adding Timeouts to Ollama API Clients
**Vulnerability:** The application was using `ollama.Client` and `ollama.list()` without configuring timeouts. If the Ollama server became unresponsive, the main application threads would block indefinitely, leading to a Denial of Service (DoS).
**Learning:** External LLM APIs can hang indefinitely. While `ollama.list()` is convenient, it doesn't allow configuring timeouts easily in module-level calls. Instantiating a local `ollama.Client(host=..., timeout=...)` is required. Furthermore, when configuring timeouts for models, a larger timeout (e.g., 120 seconds) is needed to account for "cold starts" (loading model weights into VRAM) compared to simple API calls like listing models (e.g., 30 seconds).
**Prevention:** Always instantiate `ollama.Client` with an explicit `timeout` parameter and use its instance methods (e.g., `.list()`, `.chat()`) rather than module-level functions to ensure all network requests have an upper bound on blocking time.
