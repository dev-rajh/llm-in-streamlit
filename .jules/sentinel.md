...
## 2026-06-29 - Context Poisoning / Prompt Injection via Sequential Replace
**Vulnerability:** The application was constructing LLM prompts using sequential string replacements (`template.replace("{context}", context).replace("{question}", query)`).
**Learning:** If the `context` (extracted from user-uploaded PDFs) contains the string `{question}`, the subsequent `.replace("{question}", query)` will unintentionally replace that injected string with the user's query. This allows an attacker to poison the context and manipulate the prompt structure, leading to prompt injection vulnerabilities.
**Prevention:** Avoid sequential string replacements when constructing templates with untrusted inputs. Use Python's built-in `format` with explicit keyword arguments (`template.format(context=context, question=query)`), which does not perform recursive or sequential evaluation on the interpolated values.

## 2026-06-29 - Context Poisoning and DoS via String Replacement and Formatting
**Vulnerability:** The application was constructing LLM prompts using sequential string replacements (`template.replace("{context}", context).replace("{question}", query)`). Additionally, standard formatting strings (`.format`) were considered, which crashes the application if arbitrary user inputs contain unescaped curly braces.
**Learning:** Sequential `.replace("{question}", query)` causes prompt injection if the user's PDF contains `{question}`. Conversely, `template.format()` causes a Denial of Service (application crash) if the template text contains unescaped curly braces from untrusted inputs.
**Prevention:** Construct templates using `string.Template(template).safe_substitute(context=context, question=query)`. This avoids recursive prompt injection evaluation, prevents crashes on unexpected format string sequences, and handles variable interpolations securely.
