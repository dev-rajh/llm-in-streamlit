...
## 2026-06-25 - Missing Rate Limiting on Interactive Endpoints (DoS)
**Vulnerability:** The application was not rate-limiting input via `st.chat_input`, allowing a user to send an unlimited number of requests to the backend language model rapidly.
**Learning:** In Streamlit applications where interactive elements trigger heavy backend processing or external API calls (like sending prompts to an LLM), unbounded interactive access can be exploited for application-layer Denial of Service (DoS) and API spamming. This can exhaust server resources, LLM VRAM, or trigger upstream API rate limits, impacting all users.
**Prevention:** Implement application-layer rate limiting on interactive endpoints (e.g., using `time.time()` and tracking `st.session_state.last_request_time`). Halt execution gracefully via `st.stop()` and notify the user when rate limits are exceeded, preventing backend overloading.
