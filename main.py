...
def process_pdf(file, chunk_size, chunk_overlap):
    filename = None
    try:
        # Sentinel: Ensure filename is captured before write to prevent DoS (disk exhaustion) if write fails
...
class PDFHelper:
    ...
    def ask(self, uploaded_file, question):
        ...
        temp_file_path = None
        # Sentinel: Wrap in try/finally to prevent DoS via disk exhaustion if an exception occurs
        try:
